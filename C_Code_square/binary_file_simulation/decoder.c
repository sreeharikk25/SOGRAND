...
int main(int argc, char *argv[]) {
    ...
    const double R = pow((double)k / n, 2.0);
    ...
    while (fread(&byte_in, 1, 1, fin) == 1) {
        for (int i = 7; i >= 0; i--) {
            ...
            if (bits_in_buffer == codeword_block_size) {
                ...
                for (int r = 0; r < n; r++) {
                    for (int c = 0; c < n; c++) {
                        int bit = bit_buffer[r * n + c];
                        L_channel[r][c] = (bit == 0) ? llr_magnitude : -llr_magnitude;
                    }
                }
                ...
                for (int iter = 1; iter <= Imax; iter++) {
                    ...
                    for(int r=0; r<n; ++r) for(int c=0; c<n; ++c) c_HD[r][c] = 0.5 * (1 - (L_APP[r][c] > 0 ? 1 : -1));
                    int s1_sum = 0;
                    for(int r=0; r<n; ++r) {
                ...
                for (int r = 0; r < k; r++) {
                    for (int c = 0; c < k; c++) {
                        int decoded_bit = (L_APP[r][c] > 0) ? 0 : 1;
