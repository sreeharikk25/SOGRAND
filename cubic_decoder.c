...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < k; i++) {
        int row_sum = 0;
        for (int j = 0; j < n; j++) row_sum += G[i][j];
        if (row_sum % 2 != 0) { even = 0; break; }
    }
    ...
    printf("Decoding %s to %s with Eb/N0 = %.2f dB (sigma = %f)...\n", input_filename, output_filename, EbN0dB, sigma);

    int bit_buffer[codeword_block_size];
    ...
    while (fread(&byte_in, 1, 1, fin) == 1) {
        for (int i = 7; i >= 0; i--) {
            ...
            if (bits_in_buffer == codeword_block_size) {
                Tensor_d L_channel = create_tensor_d(n, n, n);
                double llr_magnitude = 2.0 / (sigma * sigma);
                for(int j=0; j<codeword_block_size; ++j) {
                    L_channel.data[j] = (bit_buffer[j] == 0) ? llr_magnitude : -llr_magnitude;
                }
                ...
                for (int iter = 1; iter <= Imax; iter++) {
                    for(int j=0; j<n*n*n; ++j) L_A.data[j] = alpha[2*iter-2] * L_E.data[j];
                    ...
                    for (int slice = 0; slice < n; slice++) for (int col = 0; col < n; col++) {
                        ...
                        for(int row=0; row<n; ++row) { set_tensor_d(L_APP, row, col, slice, vec_L_APP[row]); set_tensor_d(L_E, row, col, slice, vec_L_E[row]); }
                    }
                    ...
                    for (int slice = 0; slice < n; slice++) for (int row = 0; row < n; row++) {
                        ...
                        SOGRAND_bitSO(vec_L_APP, vec_L_E, NULL, vec_in, H, n, k, L, Tmax, thres, even);
                        for(int col=0; col<n; ++col) { set_tensor_d(L_APP, row, col, slice, vec_L_APP[col]); set_tensor_d(L_E, row, col, slice, vec_L_E[col]); }
                    }
                    if (early_termination(L_APP, G, n, k)) break;

                    for(int j=0; j<n*n*n; ++j) L_A.data[j] = alpha[2*iter-1] * L_E.data[j];
                    for(int j=0; j<n*n*n; ++j) input.data[j] = L_channel.data[j] + L_A.data[j];
                    for (int row = 0; row < n; row++) for (int col = 0; col < n; col++) {
                        for(int slice=0; slice<n; ++slice) vec_in[slice] = get_tensor_d(input, row, col, slice);
                        SOGRAND_bitSO(vec_L_APP, vec_L_E, NULL, vec_in, H, n, k, L, Tmax, thres, even);
                        for(int slice=0; slice<n; ++slice) { set_tensor_d(L_APP, row, col, slice, vec_L_APP[slice]); set_tensor_d(L_E, row, col, slice, vec_L_E[slice]); }
                    }
                    if (early_termination(L_APP, G, n, k)) break;
                }
                ...
                for (int i = 0; i < k; i++) {
                    for (int j = 0; j < k; j++) {
                        for (int l = 0; l < k; l++) {
                            int decoded_bit = (get_tensor_d(L_APP, i, j, l) > 0) ? 0 : 1;
                            byte_out = (byte_out << 1) | decoded_bit;
                            bit_count_out++;
                            if (bit_count_out == 8) {
