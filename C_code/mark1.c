...
void print_matrix_double(const char* title, double** matrix, int rows, int cols);
void print_vector_int(const char* title, int* vector, int len);
...
void SOGRAND_bitSO(double* L_APP, double* L_E, int* N_guess, double* llr, int** H_matrix, int n, int k, int L, int Tmax, double thres, int even);


int main() {
    printf("=========================================================\n");
    printf("     SOGRAND C Implementation (with Single Bit-Flip)     \n");
    printf("=========================================================\n\n");

    const int k = 4;
    const int n = 7;

    printf("--- Using (%d, %d) Code ---\n\n", n, k);
    srand(time(NULL));
    ...
    print_matrix_int("Original Message (u):", u, k, k);
    ...
    for (int col = 0; col < n; col++) {
        ...
        for(int row = 0; row < n; row++) {
            c[row][col] = temp_row[row];
        }
    }
    print_matrix_int("Encoded Codeword (c):", c, n, n);
    ...
    printf("--- Introducing a single bit-flip error at Row %d, Col %d ---\n\n", flip_row, flip_col);


    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            y[i][j] = x[i][j] + sigma * normal_dist_rand();
            L_channel[i][j] = 2.0 * y[i][j] / (sigma * sigma);
        }
    }
    ...
    for(int i=0; i<n; ++i) for(int j=0; j<n; ++j) L_E[i][j] = 0.0;
    ...
    for (int i = 1; i <= Imax; i++) {
        ...
        for(int r=0; r<n; ++r)
          for(int cl=0; cl<n; ++cl)
            input_row_matrix[r][cl] = L_channel[r][cl] + alpha[2*i-2] * L_E[r][cl];
        for (int row = 0; row < n; row++) {
            SOGRAND_bitSO(&L_APP[row][0], &L_E[row][0], &N_guess, &input_row_matrix[row][0], H, n, k, L, Tmax, thres, even);
        }
        ...
        for (int col = 0; col < n; col++) {
            ...
            for(int r=0; r<n; ++r)
              input_col_vec[r] = input_col_matrix[r][col];
        ...
        int s1_sum = 0;
        int s2_sum = 0;
        int temp_syndrome_row[n-k];

        for(int r=0; r<n; ++r) {
            for(int s_row=0; s_row < n-k; ++s_row) {
                int syn_bit = 0;
                for(int c=0; c < n; ++c) {
                    syn_bit += c_HD[r][c] * H[s_row][c];
                }
                s1_sum += syn_bit % 2;
            }
        }
        ...
        if (s1_sum == 0) {
            printf("\nEarly termination: Found valid codeword after %d iterations.\n", i);
            break;
        }
    }
    printf("\nDecoding finished after %d iterations.\n\n", final_iter);
    ...
    print_matrix_int("Decoded Message (uhat):", uhat, k, k);
    ...
    printf("=============================================\n");
    if (errors == 0) {
        printf("Result: SUCCESS (Error was corrected)\n");
    } else {
        printf("Result: FAILURE (%d bit errors in decoded message)\n", errors);
    }
    printf("=============================================\n");
...
void SOGRAND_bitSO(double* L_APP, double* L_E, int* N_guess, double* llr, int** H_matrix, int n, int k, int L, int Tmax, double thres, int even) {
    ...
    for(int i=0; i<n; ++i) {
        p0[i] = p0[i] + p_notinlist * pp0[i];
        p1[i] = p1[i] + p_notinlist * pp1[i];
    }
...
void getGH_sys_CRC(int n, int k, int** G, int** H) {
    ...
    if (r == 3) hex_poly = "0x5";
        ...
        fprintf(stderr, "Error: (n, k) = (%d, %d) is not supported in this simplified version.\n", n, k);
        exit(1);
    }
    ...
    for (int i = 0; i < k; i++) {
        for (int j = 0; j < k + r; j++) msg_poly[j] = 0;
        msg_poly[i] = 1;
        for (int j = 0; j < k; j++) {
            if (msg_poly[j] == 1) {
...
void sogrand_main_logic(double *chat, double *score, double *T, double *curL, double *pNL, double *APP, double *llr, uint8_t *H, uint64_t n, uint64_t s, int32_t IC, uint64_t L, uint64_t Tmax, double thres, uint8_t even){
    ...
    if (IC < 0){
        if (round((double)n/2) - 1 > 0) {
            double beta = ( absL[ (uint64_t)round((double)n/2) - 1] - absL[0] ) / ( round((double)n/2) - 1 );
            if (beta > 1e-9) IC = findMax( (int32_t)round( absL[0]/beta -1 ), 0 );
            else IC = 0;
        } else {
            IC = 0;
        }
    ...
    if (ParityCheck(c, H, n, s) == 1){
        ...
        if ( (score[3] > thres) || (cur_L == L) ){
            ...
            free(perm); free(cHD); free(TEP); free(c); free(absL);
            free(u); free(D); free(d);
            return;
        }
    }
    int32_t w = 0;
...
void QuickSort (double *a, size_t *perm, uint64_t n) {
    ...
    for (i = 0, j = n - 1;; i++, j--) {
        ...
        if (i >= j) break;
        t = a[i]; a[i] = a[j]; a[j] = t;
        tt = perm[i]; perm[i] = perm[j]; perm[j] = tt;
    }
    QuickSort(a, perm, i);
...
double getLConf(double *pNL, double P_notGuess, uint64_t cur_L, double *score, uint64_t s, uint8_t even) {
    ...
    pNL[0] = P_negative;
    if (P_positive + P_negative == 0) return 1.0;
...
void mountain_build(int32_t *u, int32_t k, int32_t w, int32_t W1, int32_t n1){
    ...
    for(size_t i = 0; i < w; i++)
        W2 -= u[i];
    if (n1 - u[k] == 0) return;
    uint64_t q = (uint64_t)floor( (double)W2 / (n1 - u[k]) );
...
void print_vector_int(const char* title, int* vector, int len) {
    printf("%s [", title);
    for(int i=0; i<len; ++i) printf(" %d", vector[i]);
    printf(" ]\n\n");
}
int** create_int_matrix(int rows, int cols) {
